...
module.exports.getLista = async () => {
    var query = `SELECT distinct ?ator ?nome ?sexo (COUNT(?filme) AS ?numFilmes) WHERE {
        ?ator rdf:type :Ator .
        ?ator :nome ?nome .
        ?ator :sexo ?sexo .
        ?ator :atuou ?filme .
    }
    GROUP BY ?ator ?nome ?sexo
...
module.exports.getAtor = async (id) => {
    var query = `SELECT ?nome ?sexo ?filmeId ?filmeNome ?filmePopularidade ?filmeDataLancamento ?personagemNome ?personagemId
    ?filmeRealizadoId ?filmeRealizadoNome ?filmeRealizadoPopularidade ?filmeRealizadoDataLancamento ?filmeProduzidoId
    ?filmeProduzidoNome ?filmeProduzidoPopularidade ?filmeProduzidoDataLancamento ?filmeEscritoId ?filmeEscritoNome
